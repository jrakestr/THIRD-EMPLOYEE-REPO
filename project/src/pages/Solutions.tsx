...
import { getCaseStudiesBySolution } from '../data/caseStudies';
...
const Solutions = () => {
  ...
  const getCaseStudyForSolution = (solutionId: string) => {
    const studies = getCaseStudiesBySolution(solutionId);
    if (studies.length > 0) {
      const primaryStudy = studies[0];
      return {
        caseStudy: `${primaryStudy.title} - ${primaryStudy.description.split('.')[0]}.`,
        caseStudyLink: primaryStudy.link
      };
    }
    return { 
  ...
  const solutions = [
    {
      ...
      ...getCaseStudyForSolution("Mobility as a Service"),
    ...
    },
    {
      ...
      ...getCaseStudyForSolution("Demand-Responsive Transportation"),
